...
/// Sentinel struct name recognised by [`Serializer`](struct.Serializer.html):
/// when a value is serialized via `serialize_newtype_struct` with this name, its
/// `serialize_str` output is written to the output **verbatim** (no JSON escaping).
/// This lets callers splice pre-serialized, already-valid JSON fragments into the
/// output without re-walking them through serde.
pub const RAW_VALUE_TOKEN: &str = "$jetstream_turbo_raw";

/// A serde `Serializer` whose `serialize_str` writes its argument verbatim.
/// All other entry points are forwarded to the wrapped serializer (so misuse
/// still yields valid JSON); compound types are impossible here.
struct RawValueSerializer<'a, W: Write>(&'a mut Serializer<W>);

impl<'a, W: Write> ser::Serializer for RawValueSerializer<'a, W> {
    type Ok = ();
    type Error = Error;
    ...
    fn serialize_seq(self, _len: Option<usize>) -> Result<Self::SerializeSeq, Self::Error> {
        Err(Error::generic(ErrorType::ExpectedArray))
    }
    ...
    fn serialize_map(self, _len: Option<usize>) -> Result<Self::SerializeMap, Self::Error> {
        Err(Error::generic(ErrorType::ExpectedMap))
    }
    fn serialize_struct(
        self,
        _name: &'static str,
    ...
    fn serialize_struct_variant(
        ...
        Err(Error::generic(ErrorType::ExpectedEnum))
    }
    ...
    fn serialize_str(self, value: &str) -> Result<Self::Ok, Self::Error> {
        iomap!(self.0.write(value.as_bytes()))
    }
    ...
    where
        T: ?Sized + serde_ext::Serialize,
    {
        Err(Error::generic(ErrorType::InternalError(
            crate::InternalError::TapeError,
        )))
    }
    ...
    W: Write,
{
        ...
        T: ?Sized + serde_ext::Serialize,
    {
        if name == RAW_VALUE_TOKEN {
            // Splice pre-serialized JSON verbatim: the value's `serialize_str`
            // output (a &str) is written without escaping.
            return value.serialize(RawValueSerializer(&mut *self));
        }
        value.serialize(self)
    }
