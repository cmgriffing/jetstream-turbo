...
- Serialize breakdown (per 10k): record (OwnedValue) ~1.1ms (2x slower/byte than envelope — Cow-key serde walk), envelope ~1.8ms, metadata ~1.8ms, alloc ~0.5ms. The record raw-splice prize (~2.1ms incl. parse-side build) is definitively blocked — serde/simd-json have no raw-emit hook and OwnedValue is the fastest supported representation.
