        ...
        if databases.len() > max_databases {
            ...
            for (db_name, db_path) in to_remove {
                ...
                if let Err(e) = Self::remove_database_files(&db_path).await {
                    warn!("Failed to remove database file {}: {}", db_path.display(), e);
                }
            }
        }
...
mod tests {
    ...
    async fn test_database_listing() {
        ...
        let databases = DatabaseRotator::list_databases(temp_dir.path()).await.unwrap();
        
        assert_eq!(databases.len(), 2);
        assert!(databases.iter().any(|(name, _)| name == "jetstream_123456789.db"));
        assert!(databases.iter().any(|(name, _)| name == "jetstream_123456788.db"));
