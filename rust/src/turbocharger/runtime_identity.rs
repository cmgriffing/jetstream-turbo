...
impl PreviousTerminationClass {
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::None => "none",
            Self::ControlledMemoryExit => "controlled_memory_exit",
            Self::CgroupOom => "cgroup_oom",
            Self::GlobalOom => "global_oom",
...
#[derive(Debug, Clone, Copy, Serialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum PreviousTerminationEvidenceState {
    Available,
    Missing,
    Malformed,
    Stale,
...
impl PreviousTerminationEvidenceState {
    pub const fn as_str(self) -> &'static str {
        match self {
...
fn parse_release_identifier(value: Option<&str>) -> ReleaseIdentityDiagnostics {
    ...
    ReleaseIdentityDiagnostics {
        availability: if identifier.is_some() {
...
fn load_previous_termination(path: &Path, now: SystemTime) -> PreviousTerminationDiagnostics {
    ...
    if unix_seconds(now).saturating_sub(captured_at_unix_seconds) > MAX_EVIDENCE_AGE.as_secs() {
        return unavailable(
            PreviousTerminationEvidenceState::Stale,
            Some(captured_at_unix_seconds),
        );
    }
...
fn unavailable(
    state: PreviousTerminationEvidenceState,
...
mod tests {
    ...
    fn missing_evidence_is_explicit() {
        assert_eq!(
            load(None).previous_termination.state,
            PreviousTerminationEvidenceState::Missing
        );
    }
    ...
    fn valid_evidence_exposes_only_bounded_fields() {
        ...
        assert_eq!(
            identity.previous_termination.classification,
            Some(PreviousTerminationClass::CgroupOom)
        );
    ...
    fn malformed_evidence_is_not_partially_exposed() {
        ...
        assert_eq!(
            identity.previous_termination.state,
            PreviousTerminationEvidenceState::Malformed
        );
